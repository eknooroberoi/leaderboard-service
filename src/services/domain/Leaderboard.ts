...
class Leaderboard implements ILeaderboard{
    ...
    constructor(queueImpl: IQueueRepo, databaseImpl: IDatabaseRepo, cacheImpl: ICacheRepo) {
    ...
    async getTopScores(gameId:string, limit:number, consistentRead: boolean): Promise<TopScoresDTO | null>{
        ...
        if (!consistentRead){
            ...
            const cachedVal: TopScoresDAO | null = await this.cacheImpl.get(cacheKey);
            if (cachedVal !== undefined && cachedVal !== null){ // If found return
                return cachedVal;
            }
        }
