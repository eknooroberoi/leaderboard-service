...
export default class CacheRepo implements ICacheRepo{
    ...
    static topScoresSchemaValidate: ValidateFunction<TopScoresDAO> = ajv.addSchema(DatabaseRepo.topScorersSchema).compile(CacheRepo.topScoresSchema);
    ...
    async get(key: string): Promise<TopScoresDAO | null>{
        ...
        if (val !== undefined && val !== null) {
            try {
                ...
                console.error(`Error validating value returned from memcached. Err: ${error}`);
            }
        }
        return wrapInPromise(null);
    }

    async setAsync(key: string, value: TopScoresDAO, ttl?: number): Promise<void>{
